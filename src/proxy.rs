...
/// * `ha_url`  – base URL of the HA instance, e.g. `http://homeassistant.local:8123`
/// * `token`   – bearer token read from `.token`
...
pub async fn forward(
    ...
    let upstream_status = StatusCode::from_u16(response.status().as_u16()).unwrap_or(StatusCode::BAD_GATEWAY);
